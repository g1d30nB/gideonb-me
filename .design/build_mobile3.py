...
O.append(B(f'<div style="display:flex;align-items:center;gap:9px;padding-top:var(--s3);border-top:2px solid {AD};margin-bottom:var(--s3);">'
 ...
 f'<div style="border-top:1px solid var(--night-rule);padding-top:var(--s4);display:flex;gap:11px;align-items:flex-start;">'
 f'{D.dot(TD,"ring",10)}<p style="font-size:14px;line-height:1.7;color:{TD};margin:-4px 0 0;">The blueprints, the reusable service patterns and the playbooks were built by the service designers who were hired to make them. What I did was make the case for a capability nobody had asked for, and hire into it.</p></div>'))
...
NAMES=["Mobile.dc.html","MobCaseOneA.dc.html","MobCaseOneB.dc.html","MobCaseTwoA.dc.html","MobCaseTwoB.dc.html","MobCaseThree.dc.html"]
CONT=('<div style="background:var(--paper-2);padding:8px 22px;">'
      '<p class="lbl" style="color:var(--faint);font-size:9.5px;">{}</p></div>')
LABELS=["","Continues from the opening","Case one continues","Continues from case one","Case two continues","Continues from case two"]
segs=[[]]
for item in O[1:]:
    if item=="@@SPLIT@@": segs.append([])
    else: segs[-1].append(item)
assert len(segs)==len(NAMES), f"{len(segs)} segments, {len(NAMES)} names"
for name,label,seg in zip(NAMES,LABELS,segs):
    head=CONT.format(label) if label else ""
    body='<div style="width:390px;box-sizing:border-box;background:var(--paper);">'+head+"\n".join(seg)+'</div>'
    open(name,"w").write(HEAD+body+"\n"+TAIL)
print("mobile pages ok:", ", ".join(NAMES))
